...
export default function Details({ params }) {
  ...
  if (!model) {
      console.error("Model not found or modelId is invalid");
      return null;
  }
  ...
  const handleBuyClick = async () => {
    ...
    console.log("Principal ID:", authContext.principal);
    console.log("Model ID:", id); // Use id instead of modelId
        ...
        if (connected) {
          ...
          try {
            const response = await window.ic.plug.requestTransfer({
              to: "zypwk-mxz6q-v42wq-bmz4u-34lgo-efsfw-iivlf-v5sss-ee5qy-5qf2b-bqe",
              amount: 1_000_000,
            });
            console.log("Transfer response:", response);
  
            await recordPurchaseInSupabase(authContext.principal, id); // Use id instead of modelId
            const model = models.find((m) => m.modelId === id); // Use id instead of modelId
        ...
        <div className="container grid grid-cols-10 items-center gap-4 3xl:gap-8 pb-12">
          <div className="col-span-10 lg:col-span-6 grid grid-cols-5 items-stretch gap-6">
            <div className="relative max-lg:h-[220px] col-span-5 lg:col-span-2 py-2">
              <img
                src={model?.modelImg || "/fallback-image.png"}
            ...
            <ShortDesc
              ...
              shortDes={model?.shortDes || "No description available"}
              likes={model?.likes || "0"}
...
<div className="container flex items-center gap-8 py-2 whitespace-nowrap max-md:overflow-x-scroll">
          {tabs.map((item, index) => (
            <div
              key={index}
